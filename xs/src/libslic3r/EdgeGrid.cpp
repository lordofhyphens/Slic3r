...
void
EdgeGrid::Grid::create(const Polygons &polygons, coord_t resolution)
{
    // Count the contours.
    size_t ncontours = 0;
...
void
EdgeGrid::Grid::create(const ExPolygon &expoly, coord_t resolution)
{
    // Count the contours.
    size_t ncontours = 0;
...
void
EdgeGrid::Grid::create(const ExPolygons &expolygons, coord_t resolution)
{
    // Count the contours.
    size_t ncontours = 0;
...
void
EdgeGrid::Grid::create(const ExPolygonCollection &expolygons, coord_t resolution)
{
...
void
EdgeGrid::Grid::create_from_m_contours(coord_t resolution)
{
...
struct PropagateDanielssonSingleStep
{
    ...
    inline void
    operator()(int r, int c, int addr_delta)
    {
...
struct PropagateDanielssonSingleVStep3
{
    PropagateDanielssonSingleVStep3(float *aL, unsigned char *asigns, size_t astride, coord_t aresolution) :
        L(aL), signs(asigns), stride(astride), resolution(aresolution) {}
    inline void
    operator()(int r, int c, int addr_delta, bool has_l, bool has_r)
    {
        size_t addr = r * stride + c;
        if ((signs[addr] & 2) == 0)
        {
            ...
            for (size_t i = cell.begin; i != cell.end; ++i)
            {
                ...
                for (int corner_y = -1; corner_y < 3; ++corner_y)
                {
                    ...
                    for (int corner_x = -1; corner_x < 3; ++corner_x)
                    {
                        ...
                        Slic3r::Point
                            pt(m_bbox.min.x + corner_c * m_resolution, m_bbox.min.y + corner_r * m_resolution);
                        ...
                        if (t_pt < 0)
                        {
                            ...
                            if (dabs < d_min)
                            {
                                ...
                                int64_t t2_pt =
                                    int64_t(v_seg_prev.x) * int64_t(v_pt.x) + int64_t(v_seg_prev.y) * int64_t(v_pt.y);
                                if (t2_pt > 0)
                                {
                                    ...
                                    int64_t det = int64_t(v_seg_prev.x) * int64_t(v_seg.y)
                                        - int64_t(v_seg_prev.y) * int64_t(v_seg.x);
                                    assert(det != 0);
                                    d_min = dabs;
...
float
EdgeGrid::Grid::signed_distance_bilinear(const Point &pt) const
{
...
bool
EdgeGrid::Grid::signed_distance_edges(const Point &pt,
                                      coord_t search_radius,
                                      coordf_t &result_min_dist,
                                      bool *pon_segment) const
{
    BoundingBox bbox;
            ...
            for (size_t i = cell.begin; i < cell.end; ++i)
            {
                ...
                if (t_pt < 0)
                {
                    ...
                    if (dabs < d_min)
                    {
                        ...
                        Slic3r::Point v_seg_prev = p0.vector_to(p1);
                        int64_t
                            t2_pt = int64_t(v_seg_prev.x) * int64_t(v_pt.x) + int64_t(v_seg_prev.y) * int64_t(v_pt.y);
                        if (t2_pt > 0)
                        {
                            // Inside the wedge between the previous and the next segment.
                            d_min = dabs;
                            // Set the signum depending on whether the vertex is convex or reflex.
                            int64_t det =
                                int64_t(v_seg_prev.x) * int64_t(v_seg.y) - int64_t(v_seg_prev.y) * int64_t(v_seg.x);
                            assert(det != 0);
                            sign_min = (det > 0) ? 1 : -1;
                            on_segment = false;
                        }
...
bool
EdgeGrid::Grid::signed_distance(const Point &pt, coord_t search_radius, coordf_t &result_min_dist) const
{
...
Polygons
EdgeGrid::Grid::contours_simplified(coord_t offset) const
{
