...

{
    ...
    my $test = sub {
        ...
        diag("\nLayer Height: " , $config->layer_height,"\nFirst Layer Height: " , $config->first_layer_height);
        diag("\nSupport Z slices: " , join(", ", @$support_z), ' \n');
        diag("\n contact_z: ", join(", ", @contact_z));
        diag("\n top_z: ", join(", ", @top_z));
        diag("\n Support contact distance: ", $config->support_material_contact_distance);
        diag("\n Support Extruder : ", $config->support_material_extruder);
        diag("\n nozzle dmr: ", join(", ", @{($config->nozzle_diameter)}));
        diag("\n Raft layers: ", ($config->raft_layers));
